...
ROOM_PROGRAM: dict[str, RoomRequirement] = {
    # 📐 參考:台灣**一般住宅(公寓)**的臥室大小(使用者 2026-08-20 提供)
    #       單人房 / 次臥   2~3 坪 = 6.6~9.9 ㎡
    #       雙人房 / 主臥   3~5 坪 = 9.9~16.5 ㎡   (1 坪 ≈ 3.3 ㎡)
    #
    # ⚠️ **這組數字沒有拿來當這裡的上限,是刻意的**(使用者 2026-08-20 定調
    #    「房間大一點可以」)。因為它描述的是公寓,而這個專案主力做**透天**:
    #    19×13m 的透天每層 110㎡,次臥做到 4 坪是正常的,不是浮濫。
    #    照公寓的尺抓死,會逼生成端把臥室切小、多出來的空間再湊成別的房間 ——
    #    那是動兩帶式面積分配的大工程,只為了讓本來就沒問題的房間不亮黃燈。
    #
    #    真要做對,是「尺規依建築類型切換」(公寓一套、透天一套),不是二選一。
    #    目前沒做,所以 room_oversize 對透天而言是**偏鬆**的 —— 講給人聽的時候
    #    要講明,不要說成「我們的房間都符合一般住宅尺寸」。
