...
# ❌ 走錯過的路(2026-09-04,已拿掉):**濕區鋪地磚格線網**。
# 書上的符號對照確實有「地磚、磁磚 = 格線網」,實作出來也很順,但**出圖一看就
# 知道不能留** —— 浴廁只有 1.5×2.6m,300mm 一格的格線把馬桶、洗手台、淋浴間
# 整個蓋掉,連室名與面積都看不見。真實圖只在需要標材質的局部詳圖畫,不會鋪在
# 1:100 的平面上。**加分項不得讓原本好好的東西壞掉** —— 家具比地板材重要。
# (要做的話該走「材質標註引線」那條路,不是鋪滿格線。)


# ❌ 走錯過的路(2026-09-04,已拿掉):**儲藏室房間畫一條對角線**。
# 兩個錯:①判準寫成「這間房沒有家具才畫」,而 FURNITURE_PROGRAM["storage"] 一定
# 會擺吊衣桿 → **這條規則永遠不會生效**(本檔「加了一個會永遠成立的否決條件」);
# ②更根本的是**符號用錯地方**:書上的「收納空間 = 矩形+對角線」講的是**櫃子**
# (我們的 wardrobe / 冰箱圖塊早就這樣畫了),而儲藏室/更衣室是走得進去的房間,
# 書上另有「走道式衣櫥」的符號 —— 那件事由室內的吊衣桿家具表達。
