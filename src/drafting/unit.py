...
def one_room_unit() -> UnitSpec:
    """1 房型單元 4m×6m(局部座標;走廊在南側 y=0,對外採光在北 y=6000)。

    西南角浴廁 1.8×2.0,其餘為居室;入口門在南牆中央、對外窗在北牆中央。
    ...
    walls = [
        # 0 南牆(走廊側,RC150):入口門(中央)
        Wall((0, 0), (4000, 0), EXT, openings=[Opening(2000, 900, "door")]),
        # 1 北牆(對外,RC150):窗(中央)
